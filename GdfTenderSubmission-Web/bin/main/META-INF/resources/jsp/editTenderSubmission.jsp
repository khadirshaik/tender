...
<portlet:resourceURL id="downloadTechBidsFile"
	var="downloadTechBidsFile" />
<portlet:resourceURL id="downloadCountryProdRegFile"
	var="downloadCountryProdRegFile" />
<portlet:resourceURL id="saveTenderSubDetails"
	var="saveTenderSubDetails" />
<portlet:resourceURL id="submitTenderSubDetails"
	var="submitTenderSubDetails" />
<portlet:resourceURL id="cancelTenderSubDetails"
	var="cancelTenderSubDetails" />
<%@ page
...
<form name="tenderSubmissionForm" id="tenderSubmissionForm" method="post" action="${submitTenderActionURL}">
			...
			<div class="col-sm-12">
				...
				<div class="row">
					<div class="col-sm-12  text-right mt-4">
						<a class="linkPrimary"
							onclick="downloadTechBidsFile('<%=downloadTechBidsFile.toString()%>')">Download
							Technical Bids</a> <a class="linkPrimary"
							onclick="downloadCountryProdRegFile('<%=downloadCountryProdRegFile.toString()%>')">Download
				...
				<div class="row">
					<div class="col-sm-5 form-group  upload-section">
						<label>Upload copy of the GDF tender technical bids
							certificate</label>
						...
						<div class="input-group multiInputGroup">
							...
							<div class="input-group-btn">
								<span class="fileUpload btn btn"> <span
									class="upl upload">Choose a file</span>
								</span>
								<%-- btn-orange --%>
							</div>
					...
					<div class="col-sm-12">
						<div class="validation submissionProofFile"
							style="margin-left: 0px;">Incorrect file format or size!</div>
						<div class="fileLimitWarning" id="fileLimitWarning"></div>
						<span style="color: #f2a900; font-weight: bold;">Please
							choose a file, check that the file chosen is correct, if correct
							click on the Upload button. The file uploaded is presented at the
				...
				<div class="text-note">
					<p>
						<label><span class="condition-checkbox"><input
								type="checkbox" id="iAgree"></span> <span
							class="condition-text">I confirm that the product
								technical information has been verified and the information is
								up to date for the selected products submitted by me for this
								tender.<br /> I confirm that I have read the terms and
								conditions of the selected GDF tender reference number and I
								accept them<br /> I confirm that the company complies with the
								code of conduct for suppliers of GDF's contracted procurement
								agent <br /> I confirm I have the full authority to submit the
								technical Bids for the selected products on behalf of the
								company. <br /> I confirm that inserting my name in the form as
								a signature is the legal equivalent of my manual/handwritten
								signature on this form. <br /> By selecting "Submit" using any
								device, I consent to the legally binding submission of this
								technical information for the selected eligible products.
						</span></label>
					</p>
				</div>
		...
		<div class="col-sm-12 form-group regcertificate">
			...
			<ul id="certiReg">
				<c:forEach items="${regCertificateFileName}" var="payment">
					<li><a href="${payment.key}" target="_blank">
							${payment.value}</a> <c:if test="${regCertificateFileName.size()>1}">
						</c:if></li>
				</c:forEach>
			</ul>
		</div>
	...
	<!--copy content till here for tab content-->
		...
		<div class="popup" data-popup="deadLineDateTenderSubmitPopUp">
			...
			<div class="modal-header"><h5>Confirmation</h5></div>
				...
				<p>
					<a class="btn btn-primary" data-popup-close="deadLineDateTenderSubmitPopUp"
						href="#" id="cancelBtn">Ok</a>
				</p>

				<a class="popup-close" data-popup-close="deadLineDateTenderSubmitPopUp" href="#">x</a>
			</div>
		</div>
	...
	<div class="row text-center">
		<div class="btnWrapperCenter">
			...
			<button type="button" class="btn btn-save" onclick="saveTenderSubDetails('<%=saveTenderSubDetails.toString()%>')" id="savebtn">Save</button>
			<button type="button" id="submitbtn" onclick="validateForm()" style ="padding: 7px 24px !important; color: white !important;" class="btn submitbuttton" disabled>Submit</button>
